...
class CharacterList extends React.Component {
  state = {
    ...
    search: "", // Armazena o valor da pesquisa feita na Barra de Pesquisa
  ...
  render() {
    const { characters, search, isLoading } = this.state;

    // Filtra os personagens com base no que foi digitado na Barra de Pesquisa
    const findCharacters = search
      ? characters.filter((character) =>
          character.name.toLowerCase().includes(search.toLowerCase())
        )
      : characters;

    const renderedCharacters = findCharacters.map((character) => {
